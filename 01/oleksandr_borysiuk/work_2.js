...
let actors = movies.reduce(function (acc, { casts }) {
    let newCasts = casts.filter(function (actor) {
        let newActors = !acc.some(function (exActor) {
            if (exActor === actor) return actor;
        });
        return newActors;
    });
    return [...acc, ...newCasts];
